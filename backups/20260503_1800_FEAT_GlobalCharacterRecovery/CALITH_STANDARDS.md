...
- **Kural:** Yedekleme işlemi sırasında `.git` ve `backups` klasörünün kendisi dışarıda tutur.

---

## 12. OTURUM BAŞLATMA VE SENKRONİZASYON (KRİTİK)
Kullanıcı **"kuralları oku"** veya benzeri bir komut verdiğinde, AI asistanı projenin güncel durumuna tam hakim olmak için aşağıdaki 6 dosyayı MUTLAKA sırasıyla okumalı ve hafızasına (context) almalıdır:

1.  **`CALITH_STANDARDS.md`**: Proje anayasası ve çalışma prensipleri.
2.  **`DEVIR_TESLIM.md`**: En son yapılan işler ve yarım kalan görevler.
3.  **`MEVCUT_HATALAR.md`**: Acil çözülmesi gereken teknik buglar.
4.  **`SUPABASE_TABLES.md`**: Veritabanı şeması ve tablo yapıları.
5.  **`SMART_ENGINE_ALGORITMASI.md`**: Antrenman motorunun matematiksel mantığı.
6.  **`yol-haritam-profile.txt`**: Projenin vizyonu, fazları ve gelecek planları.

**Bu dosyalar okunmadan yapılan geliştirmeler "kör uçuş" kabul edilir ve yasaktır.**
