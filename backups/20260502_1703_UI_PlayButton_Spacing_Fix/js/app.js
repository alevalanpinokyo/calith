    ...
    indices.forEach(idx => {
        ...
        if (content && card) {
            if (shouldExpand) {
                ...
                // Dinamik yükseklik hesaplama (Pürüzsüz geçiş için)
                content.style.maxHeight = (content.scrollHeight + 40) + 'px';
                
                // Akıllı Odaklama: Kartı görüş alanına getir
                setTimeout(() => {
                ...
                content.style.maxHeight = '110px'; // CSS'teki varsayılan kapalı değer
            }
        }
    });
}
        ...
        if (Array.isArray(days)) {
            ...
            const renderCard = (day, i) => {
                ...
                <div class="program-day-card expanded-parent bg-calith-gray/40 border border-white/5 rounded-3xl overflow-hidden hover:shadow-2xl hover:shadow-calith-orange/5 transition-all group reveal active relative" id="day-card-${i}">
                    <div class="p-6 cursor-pointer flex items-center justify-between select-none relative" onclick="toggleDayAccordion(${i})">
                        <div class="flex items-center gap-4 pr-20"> <!-- Yazı butona çarpmasın diye sağ boşluk eklendi -->
                        ...
                        <!-- Hızlı Başlat Butonu (Absolute) -->
                        <button onclick="event.stopPropagation(); startWorkoutMode('${p.id}', ${i})" 
                                class="absolute top-5 right-24 w-11 h-11 rounded-2xl bg-calith-orange/10 border border-calith-orange/20 flex items-center justify-center hover:bg-calith-orange group/play transition-all shadow-xl shadow-calith-orange/10 z-30"
                                title="Antrenmanı Hemen Başlat">
                            <i data-lucide="play" class="w-5 h-5 text-calith-orange group-hover/play:text-black fill-current group-hover/play:fill-black"></i>
                        </button>
...
async function copyBulkReport() {
    ...
    logs.forEach(log => {
        ...
        text += `📅 ${date} • ${time} (${log.program_title || 'Özel'})\n`;
    ...
    try {
        ...
        showToast('Tüm rapor panoya kopyalandı! 🚀');
    } catch (err) {
...
function copyWorkoutToClipboard(logId) {
    ...
    if (!log) {
        ...
        return showToast('Bir şeyler eksik gibi kanka, veriyi bulamadım. Yenilemeyi dener misin? ⚡');
    }
...
async function startWorkoutMode(programId, dayIndex = 0) {
    ...
    if (!p) return showToast('Program verisi yüklenemedi kanka, bağlantını kontrol eder misin? 🕵️‍♂️');
