...
def get_nav_template(active_page):
        ...
        <div class="grid grid-cols-[auto_1fr_auto] items-center h-20 gap-4">
            ...
            <div class="flex items-center gap-3 justify-end">
                <a href="profile.html" class="{desktop_class('profile')} hidden lg:flex items-center gap-2 transition-all group drop-shadow-[0_0_8px_rgba(255,255,255,0.2)] hover:drop-shadow-[0_0_12px_rgba(255,107,53,0.6)]">
                    {user_circle_svg} <span class="tracking-wider whitespace-nowrap">PROFİLİM</span>
                </a>
    ...
    <div id="mobile-menu" class="hidden lg:hidden border-t border-white/10 bg-[#050505] shadow-2xl">
        <div class="px-5 py-6 space-y-4">
            ...
            <a href="profile.html" class="{mobile_class('profile')} block py-3 transition-colors flex items-center gap-3 border-t border-white/10 mt-4 pt-4 font-bold text-lg">{user_circle_svg} PROFİLİM</a>
            <button onclick="showAuthModal()" class="w-full text-left py-4 font-black text-white border-t border-white/10 mt-2 text-lg tracking-wider">GİRİŞ YAP / ÜYE OL</button>
        </div>
    </div>
</nav>'''
