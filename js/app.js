 ...
 * Tüm sayfalardaki <footer> etiketini dinamik olarak doldurur.
 */
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid md:grid-cols-4 gap-12 mb-12">
                <div class="md:col-span-1">
                    ...
                    <p class="text-gray-500 text-sm leading-relaxed">Profesyonel calisthenics ekipmanları ve eğitim kaynakları. Evinde spor salonu deneyimi.</p>
                </div>
                <div>
                    <h4 class="font-semibold mb-4 text-white uppercase tracking-widest text-xs">Ürünler</h4>
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-gray-400 text-sm">
                        <li><a href="about.html" class="hover:text-calith-orange transition-colors">Hakkımızda</a></li>
                        <li><a href="mailto:${links.supportEmail}" class="hover:text-calith-orange transition-colors">İletişim</a></li>
                    </ul>
                </div>
            </div>
            ...
            <div class="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center gap-6">
                <div class="flex flex-col items-center md:items-start gap-1">
                    <p class="text-gray-500 text-[10px] font-bold tracking-widest uppercase opacity-60">
                        &copy; ${config.year} CALITH. TÜM HAKLARI SAKLIDIR.
                    </p>
                    <p class="text-gray-700 text-[9px] font-black tracking-tighter uppercase">
                        DEVELOPED BY <span class="text-calith-orange">${config.developerName}</span>
                    </p>
                </div>
...
// Sayfa yüklendiğinde footer'ı bas
...
async function loadProducts() {
    ...
    if (!sb) {
        products = JSON.parse(localStorage.getItem('calith_products_fallback')) || [];
        renderShop();
        return;
    }
...
/**
 * Yapıştırılan içerikten inline style, Tailwind class ve gereksiz attributeâ€™ları temizler.
...
function sanitizeContent(html) {
    ...
    function cleanNode(node) {
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            // style ve class gibi zararlı attributeâ€™ları temizle
...
async function loadProfileData(user) {
    ...
    if (badgeEl) {
        ...
        const roleConfig = {
            admin: { label: 'âš¡ ADMİN', classes: 'bg-red-500/15 text-red-400 border-red-500/30' },
            mod: { label: 'ğŸ›¡ MOD', classes: 'bg-blue-500/15 text-blue-400 border-blue-500/30' },
            premium: { label: 'ğŸ‘‘ PREMİUM', classes: 'bg-yellow-500/15 text-yellow-400 border-yellow-500/30' },
...
function renderWorkoutLogs(logs, isFilterOnly = false) {
    ...
    // Dashboard ve Filtreleri sadece ilk yüklemede (veya tam yenilemede) render et
    if (!isFilterOnly) {
        renderHistoryDashboard(logs);
        renderHistoryFilters(logs);
    }

    // Gruplandırma Mantığı (Ay Bazlı)
    const grouped = {};
    logs.forEach(log => {
                            ...
                            <div class="group relative bg-white/5 border border-white/5 rounded-2xl p-4 hover:bg-white/[0.08] hover:border-calith-orange/30 transition-all cursor-pointer overflow-hidden" onclick="showWorkoutLogDetail('${log.id}')">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex-1 min-w-0 flex flex-col gap-2">
                                        ...
                                        <!-- Middle Row: Title -->
                                        <div>
                                            <h4 class="font-display text-lg sm:text-xl font-bold text-white uppercase tracking-wide group-hover:text-calith-orange transition-colors leading-tight">${log.program_title || 'Özel Antrenman'}</h4>
                                            <p class="text-[9px] text-gray-500 font-bold uppercase tracking-[0.2em] mt-1">${log.day_name || 'Serbest Çalışma'}</p>
                                        </div>
                                    </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-white/5 border border-white/5 rounded-2xl p-5 hover:border-calith-orange/20 transition-all">
                ...
                <div class="flex items-end gap-2">
                    ...
                    <span class="text-[10px] font-bold text-calith-orange mb-1">GÜN</span>
                </div>
            </div>
            <div class="bg-white/5 border border-white/5 rounded-2xl p-5 hover:border-calith-orange/20 transition-all">
                <span class="block text-[9px] font-black text-gray-500 uppercase tracking-widest mb-2">FORM KALİTESİ</span>
                <div class="flex items-end gap-2">
                    <span class="text-2xl font-black text-white">%${successRate}</span>
                    <span class="text-[10px] font-bold text-green-500 mb-1">TEMİZ</span>
                </div>
            </div>
            <div class="bg-white/5 border border-white/5 rounded-2xl p-5 hover:border-calith-orange/20 transition-all">
            ...
            <div class="bg-white/5 border border-white/5 rounded-2xl p-5 hover:border-calith-orange/20 transition-all">
                <span class="block text-[9px] font-black text-gray-500 uppercase tracking-widest mb-2">AKTİF PROGRAM</span>
                <div class="flex items-end gap-2">
...
function renderHistoryFilters(logs) {
    ...
    // Ay listesini oluştur (Benzersiz aylar)
    const months = [];
    logs.forEach(log => {
        const d = new Date(log.created_at);
        const m = d.getMonth();
    ...
    container.innerHTML = `
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
                <button onclick="filterHistory('all')" class="history-filter-btn active px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest bg-calith-orange text-white transition-all whitespace-nowrap">TÜMÜ</button>
...
function filterHistory(range) {
    ...
    window.filteredWorkoutLogs = filtered; // Kopyalama için sakla
...
async function copyBulkReport() {
    ...
    text += `\nCalith Smart Training Engine v2 ile oluşturuldu.`;

    try {
        await navigator.clipboard.writeText(text);
        showToast('Tüm rapor panoya kopyalandı! ğŸš€');
    } catch (err) {
        console.error('Kopyalama hatası:', err);
        showToast('Kopyalayamadım kanka, tarayıcı izin vermedi.');
    }
}
...
function copyWorkoutToClipboard(logId) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(() => {
            showToast('Antrenman özeti kopyalandı! ğŸ“‹');
        }).catch(err => {
        ...
        <section id="workout-mode" class="fixed inset-0 z-[1000] bg-[#050505] hidden overflow-y-auto selection:bg-calith-orange selection:text-black">
            ...
            <div class="max-w-xl mx-auto px-6 pt-6 pb-20 relative z-10">
                        ...
                        <div class="flex flex-col items-center gap-3 w-full">
                            ...
                            <div id="workout-video-container" class="flex flex-col items-center mt-1">
                                <button id="workout-video-btn" class="hidden px-4 py-2 bg-gradient-to-r from-calith-orange to-[#ff4500] rounded-xl flex items-center gap-2 shadow-md hover:shadow-lg hover:scale-105 active:scale-95 transition-all group/vbtn border border-white/20">
                                    ...
                                    <span class="text-[9px] font-black text-white uppercase tracking-[0.15em] drop-shadow-md">Hareketi İzle</span>
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex flex-col gap-2 pt-2 mb-4">
                    ...
                    <button id="btn-next-exercise" onclick="showConfirmModal('Bu hareketi atlamak istediğine emin misin kanka?', () => nextExercise())" class="w-full bg-white/5 text-gray-500 py-3 rounded-2xl font-black text-[9px] uppercase tracking-[0.3em] border border-white/5 hover:bg-white/10 hover:text-white transition-all">SIRADAKİ HAREKET</button>
                </div>
                ...
                <div class="mb-4 pb-10 border-t border-white/5 pt-6">
                    ...
                    <div id="workout-sets-list" class="grid grid-cols-1 gap-2">
                        <div class="py-6 text-center border-2 border-dashed border-white/5 rounded-2xl">
                            <p class="text-[10px] text-gray-600 font-bold uppercase tracking-widest">Henüz set girilmedi</p>
                        </div>
                    </div>
                </div>
            </div>
...
async function startWorkoutMode(programId, dayIndex = 0) {
    ...
    if (!p) return showToast('Program verisi yüklenemedi kanka, bağlantını kontrol eder misin? ğŸ•µï¸â€â™‚ï¸');
    ...
    if (ex.skippedToEnd) {
        ...
        if (restBox) {
            ...
            banner.innerHTML = `
                ...
                <div class="flex gap-3">
                    <button onclick="clearSkippedFlag()" class="flex-1 py-4 bg-white/5 rounded-2xl text-[10px] font-black text-white uppercase tracking-widest border border-white/10 hover:bg-white/10 transition-all active:scale-95">ğŸ‹ï¸ YAPMAK İSTİYORUM</button>
...
function skipExerciseAndContinue() {
    ...
    showToast('Hareket atlandı! ğŸ‘Š');
    nextExercise();
}
...
function completeSet() {
    ...
    if (!isBW && !isTimed && weight <= 0) {
        return showToast('Kanka ağırlıklı hareket yapıyorsun, 0 KG giremezsin! ğŸ‹ï¸â€â™‚ï¸');
    }
...
function confirmExitWorkout() {
    showConfirmModal("Antrenmanı bitirmeden çıkmak istediğine emin misin kanka? Verilerin kaydedilmeyecek.", () => {
...
function initSharedUI() {
                    ...
                    <div class="mt-4 pt-4 border-t border-white/5">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div class="text-center p-2 rounded-xl border border-white/10 peer-checked:border-calith-orange peer-checked:bg-calith-orange/10 hover:bg-white/5 transition-all">
                                    <div class="text-xl mb-1">ğŸ‘‘</div><span class="text-[10px] font-bold uppercase block text-gray-400 peer-checked:text-white">İleri</span>
                                </div>
                            </label>
                        </div>
                    </div>
    ...
    // Not: Bu kısım artık ensureWorkoutOverlay() tarafından dinamik yönetiliyor.
...
async function updateExerciseBest(exerciseName, weight, reps, exerciseId = null) {
    ...
    try {
        ...
        if (existing) {
            ...
            showToast(`ğŸ“ˆ İlk rekorun kaydedildi: ${exerciseName}`);
        }
    } catch (e) {
